...
hollywood_data <- import(here::here("data", "Hollywood data.xlsx"),
                       setclass = "tibble") %>%
  mutate(ID = as.factor(ID)) %>%
  janitor::clean_names()
...
png('~/Desktop/HollywoodAcc.png', units = "in", width = 7, height = 5, res = 600)
...
ggplot() +
  ...
  geom_point(data = hollywood_data,
             aes(x = long, y = lat),
             color = "#ffd34e",
             shape = 16, 
             alpha = .8, 
...
png('~/Desktop/HollywoodAcc_v3.png', units = "in", width = 7, height = 5, res = 600)
